...
u.foldertype=3;  %[1]BART, [3] any-dir
...
uimenu(cmenu, 'Label', '<html><b><font color =fuchsia> view/open selected files', 'Callback', {@context, 'viewfiles'});
...
uimenu(cmenu, 'Label', '<html><b><font color =purple> rename files ',          'Callback'           , {@context, 'renameFiles'},'separator','on');



if u.foldertype==1
    uimenu(cmenu, 'Label', '<html><b><font color =black> select files in BART', 'Callback'     , {@context, 'selectBart'},'separator','on');
end
...
% if isempty(iSameExist)
%     hold on;
%     %    hp=plot(cp(:,1),cp(:,2),'x','color','k','tag','sel','linewidth',2);
%     for i=1:size(cp,1) %PLOT
%         hold on;
%         hp=plot(cp(i,1),cp(i,2),'x','color','k','tag','sel','linewidth',2);
%         setcontextmenu();
%     end
...
%     if  ~isempty(hsel)%  DELETE FILES
%         for i=1:length(iSameExist)
%             try;  delete(hsel(iSameExist)); end
%         end
%     end
% end
% set(gcf,'WindowButtonMotionFcn',@motion);
