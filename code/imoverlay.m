function [hF,hB] = imoverlay(B,F,climF,climB,cmap,alpha,haxes,varargin)
...
%     error('Back image has %d dimensions!\n',length(size(B)));
    B=montageout(permute(B,[1 2 4 3]));
end
if size(F,3) > 1
%     error('Front image has %d dimensions!\n',length(size(F)));
    F=montageout(permute(F,[1 2 4 3]));
end
...
set(gcf,'WindowButtonDownFcn',@clickdown)
set(gcf,'WindowButtonupFcn',@clickup)
...
function clickdown(e,e2)
us=get(gcf,'userdata');
try
    set(us.hF,'AlphaData',ones(size(us.alphadata)));%'R'
end
end
function clickup(e,e2)
us=get(gcf,'userdata');
try
    set(us.hF,'AlphaData',us.alphadata.*0);%'L'
end
