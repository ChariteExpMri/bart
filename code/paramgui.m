...
newpath=fileparts(which('paramgui.m'));
...
%% replace empty cell-rows with  large "inf+number" 
iempty=find(cellfun(@isempty,qw(:,1)));
for i=1:length(iempty)
    qw(iempty(i))={['inf' num2str(1e5+iempty(i))]};
end
...
id=regexpi2(qw3, 'inf\d');
% id=regexpi2(qw3,'x.inf');
for i=1:length(id)
    dum=qw3{id(i)};
    dum(1:regexpi(dum,'='))=[];
    dum=regexprep(dum,{'''', ';' },{'' ' '});
...
%%   parse btn
% ===============================================
hb=findobj(0,'tag','pb1');
ub=get(hb,'userdata');
if isfield(ub,'OKmode') && ub.OKmode==2
    varargout{3}='parse';
end
...
if get(e,'AltDown')==1
    ...
    if ub.OKmode==1
        set(findobj(0,'tag','pb1'),'string','parse','backgroundcolor',[1 1 0]);
        ub.OKmode=2;
        drawnow;
    elseif ub.OKmode==2
        set(findobj(0,'tag','pb1'),'string','OK','backgroundcolor',[1 1 1]);
        ub.OKmode=1;
        drawnow;
    end
    set(hb,'userdata',ub);
