...
% check mode (just renew a file)
addfiles=0;

fileschk=...
    [~isempty(regexpi2(p0.filesTP(:,1),'AVGT.nii'))
    ~isempty(regexpi2(p0.filesTP(:,1),'AVGThemi.nii'))
    ~isempty(regexpi2(p0.filesTP(:,1),'ANO.nii'))];
if sum(fileschk)~=3
    addfiles=1;
end
...
for i=1:size(tb,1)
    ...
    if FinalBSplineInterpolationOrder==0
        [w2temp pid     ]=recodeIDS('forward', w2 );
        [msg,w3temp,log]=evalc('transformix(w2temp,elxout)');
        [w3   ]=recodeIDS('back', w3temp, pid );
    else
...
for i=1:size(tb,1)
    ...
    if FinalBSplineInterpolationOrder==0
        [w2temp pid     ]=recodeIDS('forward', w2 );
        [msg,w3temp,log]=evalc('transformix(w2temp,elxout_aff)');
        [w3   ]=recodeIDS('back', w3temp, pid );
    else
...
% ===============================================
if addfiles==0
    nameout=[outtag 'HISTO_orig' '.mat' ] ;
    qorig=imread(tifname);
     qorig=(mat2gray(qorig));
    try 
       qorig=imadjust(qorig);       
    end
    fi_out=fullfile(outdir, nameout);
    v=t;
    ...
    %----ANO
    n=3;
    q=(round((pseudocolorize(((imresize(thumb{ [n ] ,1},sizp,'nearest')))))));
    q=uint8(round((q-min((q(:)))) / (max((q(:))-min(q(:)))).*255));
