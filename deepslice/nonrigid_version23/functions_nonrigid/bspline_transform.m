...
%   mode: If
%           -1: nearest interpolation (for masks, modified version)   
...
if(size(I,3)<4)
    ...
    if(nargout > 1 )
        if mode==-1
            [I,Tx,Ty]=bspline_transform_2d_doubleMod(double(O(:,:,1)),double(O(:,:,2)),I,double(Spacing(1)),double(Spacing(2)),double(mode));
        else
        ...
        if mode==-1
            I=bspline_transform_2d_doubleMod(double(O(:,:,1)),double(O(:,:,2)),I,double(Spacing(1)),double(Spacing(2)),double(mode));
