...
% noGUI-update/install function for bart-project via GITHUB (no graphical userinterface)
%% ___ INFO ___________________
% updatebart('info');     % display status information, (update status, last commit, last modified files)
% updatebart('changes');  % display last local modified files (compare last update with penultimate update) 
%% ___ UPDATES ___________________
% updatebart();   % check for updates, if available, user is asked to update via cmd-window
...
% updatebart(4,'path',pwd); % FRESH INSTALLATION to current directory
% or
% updatebart(4,'path',fullfile(pwd,'bart'))
%
%% ___ ".git"-folder is missing/deleted ___________________
% updatebart(13)  ;% re-initiate .git
%


function varargout=updatebart(code,varargin)
...
if 0
    copyfile(fullfile(pwd,'updatebart.m'), fullfile(fileparts(pwd),'updatebart.m'),'f'  )
end
...
p.pathbefore=pwd;
if code~=4
    p.path=fileparts(which('bartver'));
    if exist(p.path)==7
        cd(p.path);
    else
        disp('bart-path not found');
        return
    end
end
%% ===============================================
...
if updatecode==1 %check before
    ...
    if isempty(msg);
        ...
        cprintf('*[0    0.4471    0.7412]','bart-UPDATES FOUND - Update now?\n');
        cprintf('*[0    0.4471    0.7412]','================================\n');
...
if updatecode==13
    ...
    %git remote add origin https://github.com/pstkoch/bart
...
if updatecode==4
    if isfield(p,'path')==0 | isempty(p.path)
        error('path for new installation not defined..see help of updatebart ');
        cd(p.pathbefore);
    end
    ...
    if strcmp(pa1,'bart');%bart-path selected
        patempup=pa2;
    else
        patempup=pa;
    end
...
%     end
    ...
    msgpafinal=[' DESTINATION PATH: ' fullfile(patempup,'bart') ] ;
    if ispc==1
        msgpafinal=strrep(msgpafinal,filesep,[filesep filesep]);
    end
    cprintf([0 .5 0], [msgpafinal  '\n']);
    %% ==============[install]========================
    p2.antpath    =fullfile(patempup,'bart');
...
gitpath=fileparts(which('bartver.m'));
if isempty(gitpath)
    error('.git not found --- add bart-path first');
end
...
if strcmp(trep,tloc)~=1
    cprintf('*[1 0 1]', ['UPDATE IS SUGGESTED!    type "updatebart(2)" to update toolbox'   '\n'] );
else
...
%% ========== [1] [bart]-folder exists ==================
isPrevbartdir=0;  % previous bart dir exist
if exist(p2.antpath)==7
    isPrevbartdir=1;
    % keyboard
    disp('..[bart]-folder exist ..wait..folder will be renamed & removed');
    cd(fileparts(p2.antpath)); drawnow;
    pause(1);
    
    %disp(fullfile(antupd.patempup,'bart'));
    %disp(fullfile(antupd.patempup,'_bart'));
...
if 1
    ...
    %% NOTE the installationPath without 'bart' otherwise 'bart' is within 'bart'-dir
    %     if exist(p2.antpath)
    %         mkdir(p2.antpath);
    %     end
    %     cd(p2.antpath);
    ...
    if 1
        ...
        cprintf('*[0    0.4471    0.7412]','..CLONING BART REPO from GITHUB..please wait...\n');
        %git clone https://github.com/pstkoch/bart
        ...
        if ~isempty(strfind(msg,'unable to access')) || (exist(fullfile(p2.antpath,'.git'))~=7)
            if (exist(fullfile(p2.antpath,'.git'))~=7)
                disp('..failed to remove "bart"-directory');
...
function [msg st]=bart_gitstatus(message)
...
https='https://github.com/ChariteExpMri/bart.git';
